...
| **Natural 12** | Rally — no more checks this fight unless something far worse happens |
...
| 12 | Fearless — will never flee (golems, demons, the utterly devoted) |

Every monster stat block includes an Ml score.

---

## Retainer Morale

Retainers use the same 2d6 roll, but add modifiers:
...
## Optional: Player Character Morale

Player characters don't check morale — players decide when to run. However, hirelings, animal companions, and summoned creatures do check.

If you want to add tension, a Referee may call for a **WIS save (DC 12)** when something truly terrifying happens. Failure means the character is **Shaken** (SETBACK on attacks) until they spend a Breather recovering.
